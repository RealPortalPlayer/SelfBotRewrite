...
int main(int argc, char** argv) {
    ...
    const char* dayName = SBR_Time_GetDayName(parsedTime);
    
    BA_LOGGER_INFO("Time: (%s, %s) %i/%i/%i @ %i:%i:%i\n", monthName, dayName, parsedTime->tm_mon + 1, parsedTime->tm_mday, parsedTime->tm_year + 1900, parsedTime->tm_hour - 1, parsedTime->tm_min, parsedTime->tm_sec);
    ...
    BA_ASSERT(strcmp(dayName, "Sunday") == 0, "Wrong day name\n");
}
