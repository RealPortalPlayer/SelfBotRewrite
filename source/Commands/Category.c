...
void SBR_Category_Initialize(void) {
    SBR_ASSERT(!sbrCategoryInitialized, "Category system are already initialized\n");
    BA_LOGGER_INFO("Initializing categories\n");

    sbrCategoryInitialized = BA_BOOLEAN_TRUE;

    SBR_ASSERT(SBR_DynamicDictionary_Create(&sbrCategoryRegistered, 10), "Failed to initialize registered categories array\n");
}
...
void SBR_Category_Register(const char* name, const char* description) {
    ...
    SBR_ASSERT(BA_DynamicDictionary_AddElementToLast(&sbrCategoryRegistered, (void*) name, (void*) description), "Failed to add registered category\n");
}

void SBR_Category_Destroy(void) {
    SBR_ASSERT(sbrCategoryInitialized, "Category system is not initialized\n");
