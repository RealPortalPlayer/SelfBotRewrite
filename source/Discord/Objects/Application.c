...
SBR_DiscordApplication* SBR_DiscordApplication_Create(json_object* unparsedJsonData) {
    ...
    SBR_OBJECTCREATORHELPERS_GET_USER(bot, "", UNREQUIRED, SAME);
    SBR_OBJECTCREATORHELPERS_GET_STRING(termsOfServiceUrl, "terms_of_service_url", UNREQUIRED, NOT_SAME);
    SBR_OBJECTCREATORHELPERS_GET_STRING(privacyPolicyUrl, "privacy_policy_url", UNREQUIRED, NOT_SAME);
    ...
    // TODO: guild
    SBR_OBJECTCREATORHELPERS_GET_SNOWFLAKE(primarySkuId, "primary_sku_id", UNREQUIRED, NOT_SAME);
    SBR_OBJECTCREATORHELPERS_GET_STRING(storeSlug, "slug", UNREQUIRED, NOT_SAME);
    SBR_OBJECTCREATORHELPERS_GET_STRING(richPresenceCoverImage, "cover_image", UNREQUIRED, NOT_SAME);
    SBR_OBJECTCREATORHELPERS_GET_INTEGER(officialFlags, "flags", UNREQUIRED, NOT_SAME);
    SBR_OBJECTCREATORHELPERS_GET_INTEGER(approximateGuildCount, "approximate_guild_count", UNREQUIRED, NOT_SAME);
    SBR_OBJECTCREATORHELPERS_GET_JSON_OBJECT(redirectUris, "redirect_uris", UNREQUIRED, NOT_SAME);
    SBR_OBJECTCREATORHELPERS_GET_STRING(roleConnectionsVerificationUrl, "role_connections_verification_url", UNREQUIRED, NOT_SAME);
    SBR_OBJECTCREATORHELPERS_GET_JSON_OBJECT(tags, "", UNREQUIRED, SAME);
    // TODO: install_params
    SBR_OBJECTCREATORHELPERS_GET_JSON_OBJECT(integrationTypesConfiguration, "integration_types_config", UNREQUIRED, NOT_SAME);
    SBR_OBJECTCREATORHELPERS_GET_STRING(customInstallationUrl, "custom_install_url", UNREQUIRED, NOT_SAME);
